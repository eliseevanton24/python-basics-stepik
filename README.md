...
## Системы счисления
```python
bin(255)          # десятичное → двоичное   → '0b11111111'
oct(255)          # десятичное → восьмеричное → '0o377'
hex(255)          # десятичное → шестнадцатеричное → '0xff'

int('11111111', 2)  # двоичное → десятичное  → 255
int('377', 8)       # восьмеричное → десятичное → 255
int('ff', 16)       # шестнадцатеричное → десятичное → 255
```
...
abs(x)             # модуль числа (встроенная функция)
```
